    ...
    for config_file in config_files:
        ...
        if model_file and model_file.exists():
            ...
            if monitoring_mask.any():
                total_counter_goals_mon = final_agent_values[monitoring_mask]['Counter Conflict Goal Accomplishments'].sum()
                total_resource_conflicts_mon = final_agent_values[monitoring_mask]['Resource Conflicts'].sum()
                avg_counter_goal_per_resource_conflict_ratio_monitoring_agent = (total_counter_goals_mon / total_resource_conflicts_mon) if total_resource_conflicts_mon > 0 else 0
            else:
                avg_counter_goal_per_resource_conflict_ratio_monitoring_agent = 0
            ...
            # Calculate steps for this run as the last value of the Step/index column from model timeline
            ...
            simulation_data.append({
                ...
                'avg_counter_goal_per_resource_conflict_ratio_monitoring_agent': avg_counter_goal_per_resource_conflict_ratio_monitoring_agent,
...
def create_agent_ratio_analysis(experiment_name=None, experiment_date=None):
    ...
    }).reset_index().sort_values('consent_first_count')
    ...
    ax_gn1.errorbar((df_summary['consent_first_count'] / 1000), rc_per_step,
                    yerr=rc_per_step_sem,
                    fmt='o-', linewidth=2, markersize=8, capsize=5,
                    label='Resource Conflicts per Step', color='purple')
    ax_gn1.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=11)
    ax_gn1.set_ylabel('Resource Conflicts per Step', fontsize=11)
    ax_gn1.set_title('Resource Conflicts per Step vs Deontic/Virtue Agents Ratio', fontsize=12, fontweight='bold')
    ...
    ax_gn2.errorbar((df_summary['consent_first_count'] / 1000), cg_per_step,
                    yerr=cg_per_step_sem,
                    fmt='o-', linewidth=2, markersize=8, capsize=5,
                    label='Counter Goal Accomplishments per Step', color='brown')
    ax_gn2.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=11)
    ax_gn2.set_ylabel('Counter Goal Accomplishments per Step', fontsize=11)
    ax_gn2.set_title('Counter Goal Accomplishments per Step vs Deontic/Virtue Agents Ratio', fontsize=12, fontweight='bold')
...
def create_agent_level_analysis(experiment_name=None, experiment_date=None):
    ...
    config_columns = ['consent_first_count', 'monitoring_count', 'fifty_fifty_count', 'total_agents', 'avg_steps_overall']
    ...
    ax_idle.errorbar(x_all[cf_mask], df_agent_summary['avg_total_idle_time_consent_first_agent'][cf_mask],
                     ...
                     fmt='o-', linewidth=2, markersize=6, capsize=4, label='Deontic Agent', color='green')
    ax_idle.errorbar(x_all[m_mask], df_agent_summary['avg_total_idle_time_monitoring_agent'][m_mask],
                     yerr=df_agent_summary['avg_total_idle_time_monitoring_agent_sem'][m_mask],
                     fmt='s-', linewidth=2, markersize=6, capsize=4, label='Virtue Agent', color='blue')
    ax_idle.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=10)
    ...
    y_m_idle = (df_agent_summary['avg_total_idle_time_monitoring_agent'] / steps_idle)
    ax_idle_norm.plot(x_all[cf_mask], y_cf_idle[cf_mask], 'o-', linewidth=2, markersize=6, label='Deontic Agent', color='green')
    ax_idle_norm.plot(x_all[m_mask], y_m_idle[m_mask], 's-', linewidth=2, markersize=6, label='Virtue Agent', color='blue')
    ax_idle_norm.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=10)
    ...
    steps = df_agent_summary['avg_steps_overall'].replace(0, np.nan)

    # N1. Resource Conflicts per Agent per Step
    ax_n1 = axes_norm[0]
    y_cf_rc = (df_agent_summary['avg_resource_conflicts_consent_first_agent'] / steps)
    y_m_rc = (df_agent_summary['avg_resource_conflicts_monitoring_agent'] / steps)
    ax_n1.plot(x_all[cf_mask], y_cf_rc[cf_mask], 'o-', linewidth=2, markersize=6, label='Deontic Agent', color='green')
    ax_n1.plot(x_all[m_mask], y_m_rc[m_mask], 's-', linewidth=2, markersize=6, label='Virtue Agent', color='blue')
    ax_n1.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=10)
    ...
    y_m_cg = (df_agent_summary['avg_counter_goal_accomplishments_monitoring_agent'] / steps)
    ax_n2.plot(x_all[cf_mask], y_cf_cg[cf_mask], 'o-', linewidth=2, markersize=6, label='Deontic Agent', color='green')
    ax_n2.plot(x_all[m_mask], y_m_cg[m_mask], 's-', linewidth=2, markersize=6, label='Virtue Agent', color='blue')
    ax_n2.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=10)
    ...
    ax_cg_rc.errorbar(x_all[cf_mask], df_agent_summary['avg_counter_goal_per_resource_conflict_ratio_consent_first_agent'][cf_mask],
                     ...
                     fmt='o-', linewidth=2, markersize=6, capsize=4, label='Deontic Agent', color='green')
    ax_cg_rc.errorbar(x_all[m_mask], df_agent_summary['avg_counter_goal_per_resource_conflict_ratio_monitoring_agent'][m_mask],
                     yerr=df_agent_summary['avg_counter_goal_per_resource_conflict_ratio_monitoring_agent_sem'][m_mask],
                     fmt='s-', linewidth=2, markersize=6, capsize=4, label='Virtue Agent', color='blue')
    ax_cg_rc.set_xlabel('Deontic Agent Ratio (Deontic:All)', fontsize=10)
