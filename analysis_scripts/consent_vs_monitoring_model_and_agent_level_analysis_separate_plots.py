#!/usr/bin/env python3
"""Separate-plot export for the consent-vs-monitoring analysis."""
...
import consent_vs_monitoring_model_and_agent_level_analysis as base_analysis
...
def _prepare_simulation_summary() -> pd.DataFrame:
    ...
    metrics_to_average = [
        'consent_first_count', 'monitoring_count', 'total_agents',
    ...
    summary = summary.sort_values('monitoring_count')
    return summary
...
def create_agent_ratio_analysis_separate():
    ...
    ratio = df_summary['monitoring_count'] / df_summary['total_agents']
    ...
    ax_steps.set_xlabel('Virtue Agent Ratio (Virtue:All)', fontsize=11)
...
def create_cumulative_plots_separate():
    ...
    }).reset_index().sort_values('monitoring_count')
    ...
    for _, cfg_row in unique_cfgs.iterrows():
        ...
        if not cf_series and not m_series:
            continue
...
def _prepare_agent_summary():
    ...
    summary = mean_df.copy()
    for col in agent_metrics_to_average.stringify() if False else agent_metrics_to_average:
        summary[f'{col}_std'] = std_df[col]
        summary[f'{col}_sem'] = sem_df[col]

    summary = summary.merge(count_df, on=['experiment_name', 'agent_config'])

    config_columns = ['consent_first_count', 'monitoring_count', 'total_agents', 'avg_steps_overall']
...
def create_agent_level_analysis_separate():
    ...
    ratio = df_agent['monitoring_count'] / df_agent['total_agents']
    ...
    def save_dual_series_plot(filename, title, ylabel, cf_series, m_series, cf_sem=None, m_sem=None):
        fig, ax = plt.subplots(figsize=(7.5, 5))
