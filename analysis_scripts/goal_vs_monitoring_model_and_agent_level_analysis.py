...
experiment_date = "20251120"  # Format: YYYYMMDD
...
def create_agent_ratio_analysis(experiment_name=None, experiment_date=None):
    ...
    def _find_agent_file(prefix: str):
        ...
        if (main_data / f"{prefix}_agents.csv").exists():
            return main_data / f"{prefix}_agents.csv"
        for sub in results_dir.iterdir():
            if sub.is_dir():
                d = sub / "data"
                p = d / f"{prefix}_agents.csv"
    ...
    # NEW FIGURE: Normalized general metrics by steps (Resource Conflicts and Counter Goals per Step)
...
def create_agent_level_analysis(experiment_name=None, experiment_date=None):
    ...
    ax_idle_norm.plot(x_all[gf_mask], y_gf_idle[gf_mask], 'o-', linewidth=2, markersize=6, label='Teleological Agent', color='red')
    ...
    try:
        print("[DIAG] goal_vs_monitoring normalized-by-steps checks:")
        print("       min steps:", steps.min())
        print("       min RC GF:", df_agent_summary['avg_resource_conflicts_goal_first_agent'].min())
        print("       min RC M:", df_agent_summary['avg_resource_conflicts_monitoring_agent'].min())
        norm_gf_tmp = df_agent_summary['avg_resource_conflicts_goal_first_agent'] / steps
        norm_m_tmp = df_agent_summary['avg_resource_conflicts_monitoring_agent'] / steps
        print("       min norm RC GF:", norm_gf_tmp.min(), "min norm RC M:", norm_m_tmp.min())
    ...
    ax_n1.plot(x_all[gf_mask], y_gf_rc[gf_mask], 'o-', linewidth=2, markersize=6, label='Teleological Agent', color='red')
    ...
    ax_n2.plot(x_all[gf_mask], y_gf_cg[gf_mask], 'o-', linewidth=2, markersize=6, label='Teleological Agent', color='red')
