#!/usr/bin/env python3
"""Separate-plot export for the goal-vs-monitoring analysis."""
...
import goal_vs_monitoring_model_and_agent_level_analysis as base_analysis
...
def _prepare_simulation_summary() -> pd.DataFrame:
    ...
    metrics_to_average = [
        'goal_first_count', 'monitoring_count', 'total_agents',
...
def create_cumulative_plots_separate():
    ...
    for _, cfg_row in unique_cfgs.iterrows():
        ...
        for _, srow in seed_rows.iterrows():
            ...
            mon = adf[adf['Agent Persona'] == 'MonitoringAgent'].groupby(step_col)['Accomplished Goals'].mean().sort_index()
        ...
        if not gf_series and not m_series:
            continue
        ...
        if gf_series:
            ...
            ax.plot(gf_values.index, gf_values.values, 'r-', linewidth=4, label='Teleological Agent (per agent)')
        if m_series:
...
def _prepare_agent_summary():
    ...
    agent_metrics_to_average = [
        ...
        'avg_total_idle_time_goal_first_agent', 'avg_total_idle_time_monitoring_agent',
    ...
    config_columns = ['goal_first_count', 'monitoring_count', 'total_agents', 'avg_steps_overall']
...
def create_agent_level_analysis_separate():
    ...
    m_mask = df_agent['monitoring_count'] > 0
    steps = pd.to_numeric(df_agent['avg_steps_overall'], errors='coerce').replace(0, np.nan)

    def save_dual_series_plot(filename, title, ylabel, gf_series, m_series, gf_sem=None, m_sem=None):
        ...
        if gf_sem is not None and m_sem is not None:
            ...
            ax.errorbar(ratio[m_mask], m_series[m_mask], yerr=m_sem[m_mask],
                        ...
                        label='Preemptive Agent', color='#CCCCCC')
        else:
            ax.plot(ratio[gf_mask], gf_series[gf_mask], 's-', linewidth=3, markersize=15,
                    label='Selfish Agent', color='#000000')
            ax.plot(ratio[m_mask], m_series[m_mask], 'o-', linewidth=3, markersize=15,
                    label='Preemptive Agent', color='#CCCCCC')
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_violated_consents_monitoring_r'],
        gf_sem=df_agent['avg_violated_consents_goal_first_r_sem'],
        m_sem=df_agent['avg_violated_consents_monitoring_r_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_total_consents_monitoring_r'],
        gf_sem=df_agent['avg_total_consents_goal_first_r_sem'],
        m_sem=df_agent['avg_total_consents_monitoring_r_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_sem=df_agent['avg_consent_violation_ratio_monitoring_r_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_sem=df_agent['avg_consent_fulfillment_ratio_monitoring_r_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_violated_consents_monitoring_g'],
        gf_sem=df_agent['avg_violated_consents_goal_first_g_sem'],
        m_sem=df_agent['avg_violated_consents_monitoring_g_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_total_consents_monitoring_g'],
        gf_sem=df_agent['avg_total_consents_goal_first_g_sem'],
        m_sem=df_agent['avg_total_consents_monitoring_g_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_consent_violation_ratio_monitoring_g'],
        gf_sem=df_agent['avg_consent_violation_ratio_goal_first_g_sem'],
        m_sem=df_agent['avg_consent_violation_ratio_monitoring_g_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_consent_fulfillment_ratio_monitoring_g'],
        gf_sem=df_agent['avg_consent_fulfillment_ratio_goal_first_g_sem'],
        m_sem=df_agent['avg_consent_fulfillment_ratio_monitoring_g_sem']
    )

    # General metrics
    save_dual_series_plot(
        filename='agent_level_accomplished_goals_per_agent',
        title='Total Accomplished Goals per Agent',
        ...
        m_sem=df_agent['avg_accomplished_goals_monitoring_agent_sem']
    )

    save_dual_series_plot(
        filename='agent_level_remaining_goals_per_agent',
        title='Remaining Goals per Agent',
        ylabel='Remaining Goals per Agent',
        gf_series=df_agent['avg_remaining_goals_goal_first_agent'],
        m_series=df_agent['avg_remaining_goals_monitoring_agent'],
        gf_sem=df_agent['avg_remaining_goals_goal_first_agent_sem'],
        m_sem=df_agent['avg_remaining_goals_monitoring_agent_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_sem=df_agent['avg_resource_conflicts_monitoring_agent_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_sem=df_agent['avg_counter_goal_accomplishments_monitoring_agent_sem']
    )

    # Idle time
    save_dual_series_plot(
        filename='agent_level_total_idle_time_per_agent',
        title='Idle Time per Agent',
        ylabel='Idle Time per Agent',
        gf_series=df_agent['avg_total_idle_time_goal_first_agent'],
        m_series=df_agent['avg_total_idle_time_monitoring_agent'],
        gf_sem=df_agent['avg_total_idle_time_goal_first_agent_sem'],
        m_sem=df_agent['avg_total_idle_time_monitoring_agent_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_finished_step_monitoring_agent'],
        gf_sem=df_agent['avg_finished_step_goal_first_agent_sem'],
        m_sem=df_agent['avg_finished_step_monitoring_agent_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_longest_idle_time_monitoring_agent'],
        gf_sem=df_agent['avg_longest_idle_time_goal_first_agent_sem'],
        m_sem=df_agent['avg_longest_idle_time_monitoring_agent_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_sem=df_agent['avg_distinct_agents_interacted_r_monitoring_agent_sem']
    )
    ...
    save_dual_series_plot(
        ...
        m_series=df_agent['avg_distinct_agents_interacted_g_monitoring_agent'],
        gf_sem=df_agent['avg_distinct_agents_interacted_g_goal_first_agent_sem'],
        m_sem=df_agent['avg_distinct_agents_interacted_g_monitoring_agent_sem']
    )
